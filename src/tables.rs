...
const SCORE_DIVISOR: f32 = 2.0;
const MIN_VALID_THRESHOLD: f32 = 0.0;
...
impl Sheet {
    pub const fn new(ages: Vec<u8>, scores: Vec<u8>, results: Vec<Vec<f32>>, ascending: bool) -> Self {
        Self { ages, scores, results, ascending }
    }
    ...
    pub fn lookup(&self, age: u8, result: f32) -> Option<f32> {
        ...
        let valid_thresholds = age_results.iter()
            .enumerate()
            .filter(|(_, &threshold)| threshold > MIN_VALID_THRESHOLD);

        let score_idx = if self.ascending {
            valid_thresholds
                .filter(|(_, &threshold)| threshold <= result)
                .map(|(idx, _)| idx)
                .last()
                .unwrap_or(0)
        } else {
        ...
        if score_idx == 0 {
            ...
            if worst_threshold > MIN_VALID_THRESHOLD && ((self.ascending && result < worst_threshold)
                || (!self.ascending && result > worst_threshold))
            {
                return Some(MIN_VALID_THRESHOLD); // Score 0.0 for worse than worst
            }
        }

        Some(self.scores[score_idx] as f32 / SCORE_DIVISOR)
    }
}
...
impl AllTables {
    pub fn get_exercise<S: AsRef<str>>(&self, name: S) -> Option<(&Sheet, &Sheet)> {
        let name = name.as_ref();
        let idx = exercise::Exercise::all_exercises_ordered()
            .iter()
            .enumerate()
            .find(|(_, (ex, _))| {
                let table_name_motor6 = ex.table_name(Some(exercise::ChallengeType::Hungarofit));
                let table_name_motor4 = ex.table_name(Some(exercise::ChallengeType::HungarofitMini));

                table_name_motor6 == name || table_name_motor4 == name || ex.table_name(None) == name
            })
            .map(|(i, _)| i)?;
    ...
    pub fn get_sheet<S: AsRef<str>>(&self, name: S, male: bool) -> Option<&Sheet> {
        let name = name.as_ref();
        let idx = exercise::Exercise::all_exercises_ordered()
            .iter()
            .enumerate()
...
pub fn load_tables(data: &[u8]) -> AllTables {
    let mut offset = 0;
    let sheet_count = u32::from_le_bytes(data[offset..offset + 4].try_into().unwrap()) as usize;
    offset += 4;

    let mut sheets = Vec::with_capacity(sheet_count);

    for _ in 0..sheet_count {
        let age_count = u16::from_le_bytes([data[offset], data[offset+1]]) as usize;
        offset += 2;
        let score_count = u16::from_le_bytes([data[offset], data[offset+1]]) as usize;
        offset += 2;
        ...
        let scores = data[offset..offset + score_count].to_vec();
        offset += score_count;
        ...
        for age_idx in 0..age_count {
            ...
            for _ in 0..score_count {
                let delta_bits = u16::from_le_bytes([data[offset], data[offset + 1]]);
                offset += 2;
